...
*/
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptPembelian.jasper", "report", "::[ Transaksi Penerimaan Barang Non Medis dan Penunjang ( Lab & RO ) ]::", "select * from temporary where temporary.temp37='" + akses.getalamatip() + "' order by temporary.no", param);
        }
        ...
        if (tbDokter.getSelectedRow() > -1) {
            ...
            try {
                ...
                ps = koneksi.prepareStatement("select ipsrspembelian.tgl_beli,ipsrspembelian.no_faktur, "
                        ...
                        + "ipsrspembelian.ppn,ipsrspembelian.tagihan,ipsrspembelian.meterai,ipsrspembelian.keterangan from ipsrspembelian inner join ipsrssuplier inner join petugas inner join kodesatuan  "
                        ...
                        + " where ipsrspembelian.no_faktur like ? ");
                try {
                    ...
                    if (rs.next()) {
                        ...
                        param.put("kontakrs", akses.getkontakrs());
                        param.put("emailrs", akses.getemailrs());                        
                        param.put("suplier", Sequel.cariIsi("select ipsrssuplier.nama_suplier from ipsrssuplier inner join ipsrspembelian on ipsrssuplier.kode_suplier=ipsrspembelian.kode_suplier where ipsrspembelian.no_faktur=?",tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString()));
                        param.put("nomorfaktur", Sequel.cariIsi("select ipsrspembelian.no_faktur from ipsrspembelian where ipsrspembelian.no_faktur=?",tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString()));
                        ...
                        param.put("tanggal", akses.getkabupatenrs());
                        param.put("logo", Sequel.cariGambar("select setting.logo from setting"));
                        ...
                        try {
                            ...
                            while (rs2.next()) {
                                Sequel.menyimpan(
                                        ...
                                        + rs2.getString("nama_brng") + "','" + Valid.SetAngka(rs2.getDouble("harga")) + "','" + Valid.SetAngka(rs2.getDouble("subtotal")) + "','"
                                        + rs2.getDouble("dis") + "','" + Valid.SetAngka(rs2.getDouble("besardis")) + "','" + Valid.SetAngka(rs2.getDouble("total")) + "','"
                                        + "','','','','','','','','','','','','','','','','','','','','','','','','','','','" + akses.getalamatip() + "'", "Transaksi Penerimaan (MRS)"
                                );
                                i++;
                            }
                        } catch (Exception e) {
                        ...
                        Valid.MyReportqry("rptSuratPenerimaanNonMedis1.jasper", "report", "::[ Transaksi Penerimaan Barang (MRS) ]::", "select * from temporary where temporary.temp37='" + akses.getalamatip() + "' order by temporary.no", param);
                        this.setCursor(Cursor.getDefaultCursor());
